...
export default function CartPage() {
    ...
    const renderCartItem = (item: CartItem) => {
                    ...
                    <div className="flex-1 w-full">
                        ...
                        <div className="flex items-center justify-between mt-2">
                            <div className="flex items-center space-x-2">
                                <Label htmlFor={`quantity-${item.id}`} className="text-xs text-muted-foreground">Qty:</Label>
                                <select 
                                    id={`quantity-${item.id}`}
                                    value={item.quantity}
                                    onChange={(e) => updateItemQuantity(item.id, parseInt(e.target.value))}
                                    className="bg-background border border-input rounded text-foreground text-xs py-1 px-2"
                                >
        ...
        if (item.type === 'service') {
            return (
                 <div key={item.id} className="flex items-center space-x-6 pb-4 border-b border-border last:border-0 last:pb-0">
                    <div className="w-20 h-20 bg-secondary/50 rounded-lg border border-border flex-shrink-0 flex items-center justify-center">
    ...
    if (cartItems.length === 0) {
        return (
            <div className="min-h-[60vh] flex flex-col items-center justify-center px-4">
                <div className="glass-panel p-12 rounded-3xl text-center border border-border max-w-lg w-full">
                    <div className="w-20 h-20 bg-muted rounded-full flex items-center justify-center mx-auto mb-6">
                        <ShoppingBag className="w-10 h-10 text-muted-foreground" />
                    </div>
                    <h2 className="text-2xl font-bold text-foreground mb-2">Your cart is empty</h2>
                    <p className="text-muted-foreground mb-8">Looks like you haven't started building any gang sheets yet.</p>
                        ...
                        <div className="p-6 space-y-6">
                            {isUserLoading ? (
                                ...
                                <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4 flex items-center">
                                    ...
                                    <div>
                                        <p className="text-sm font-medium text-foreground">Logged in as {currentUser.displayName || currentUser.email}</p>
                                        <p className="text-xs text-muted-foreground">Your information has been pre-filled.</p>
                                    </div>
                                </div>
                            ) : (
                                <div className="bg-primary/10 border border-primary/20 rounded-xl p-4 transition-all">
                                    <div className="flex items-start">
                                        <div className="flex h-5 items-center">
                                            <Input
                                                ...
                                                className="h-4 w-4 rounded border-muted-foreground bg-background text-primary focus:ring-primary"
                                            />
                                        </div>
                                    ...
                                    {formData.createAccount && (
                                        <div className="mt-4 animate-in fade-in">
                                            <Label className="block text-xs font-medium text-muted-foreground mb-1">Create Password</Label>
                                            <div className="relative">
                                                ...
                                                <Input 
                                                    ...
                                                    onChange={handleInputChange} 
                                                    type="password" 
                                                    className="w-full bg-background border-input rounded-lg pl-10 pr-4 py-2.5 text-foreground focus:ring-primary focus:border-primary" 
                                                    placeholder="••••••••"
                                                />
                                            </div>
                                        </div>
                            ...
                            <div className="pt-4 border-t border-border/5">
                                 ...
                                 {isLoadingRates ? (
                                     <div className="flex items-center justify-center py-6 space-x-2 text-muted-foreground">
                                     ...
                                     <div className="space-y-3">
                                        {availableRates.map((rate) => (
                                            <Label key={rate.id} className={`flex items-center justify-between p-4 rounded-xl border cursor-pointer transition-all ${selectedRateId === rate.id ? 'bg-secondary border-primary' : 'bg-background/50 border-border hover:border-muted-foreground/50'}`}>
                                                <div className="flex items-center">
                                                    <Input 
                                                        ...
                                                        className="h-4 w-4 text-primary focus:ring-primary bg-muted border-border"
                                                    />
                                                    <div className="ml-3">
                                                        <span className="block text-sm font-medium text-foreground">{rate.carrier} {rate.service}</span>
                                                        <span className="block text-xs text-muted-foreground">{rate.deliveryDays}</span>
                                                    </div>
                                                </div>
                                     ...
                                     <div className="text-center py-4 text-sm text-muted-foreground bg-muted/50 rounded-lg border border-border">
                ...
                <div className="lg:col-span-1">
                    <div className="glass-panel p-6 rounded-2xl border border-border sticky top-24">
                        ...
                        <div className="space-y-4 mb-6 pb-6 border-b border-border text-sm">
                            ...
                            <div className="flex justify-between text-muted-foreground">
                                <span>Shipping</span>
                                <span className="text-foreground">{shippingCost === 0 && !isTestMode && hasPhysicalItems ? 'Calculated' : formatCurrency(shippingCost)}</span>
                            </div>
                            {isTestMode && (
                         ...
                         <div className="mb-6">
                            {!isTestMode ? (
                                ...
                                <div className="flex items-center justify-between bg-accent/10 border border-accent/20 p-2 rounded-lg">
                                    ...
                                    <Button onClick={handleRemoveCoupon} variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground hover:text-foreground">
