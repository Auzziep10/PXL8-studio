...
import { 
    Printer, Search, Download, 
    X, QrCode, 
    ChevronRight, User as UserIcon, DollarSign,
    Package, History, ArrowLeft, ArrowUpRight,
    ArrowUp, ArrowDown, ZoomIn, FileText,
    Filter, Database, Cloud
} from 'lucide-react';
import JSZip from 'jszip';
import FileSaver from 'file-saver';
import { ImagePreviewModal } from '@/components/ImagePreviewModal';
import { backend, isCloudEnabled } from '@/services/backend';
...
// --- AssetCard Component ---
const AssetCard: React.FC<{ item: GangSheetItem; index: number; onPreview: (url: string) => void }> = ({ item, index, onPreview }) => {
    ...
    return (
        <div className="bg-zinc-900 rounded-xl border border-white/10 overflow-hidden shadow-sm hover:border-white/20 transition-colors">
             ...
             <div className="p-8 bg-checkerboard-dark flex justify-center items-center relative min-h-[300px] group cursor-zoom-in" onClick={() => displayUrl && onPreview(displayUrl)}>
                {displayUrl ? (
                    <>
                        <img 
                            src={displayUrl} 
                ...
                <div className="absolute top-4 left-4 flex gap-2">
                     <span className={`px-2 py-1 rounded text-xs font-medium backdrop-blur-md border border-white/10 shadow-lg ${showOriginal ? 'bg-blue-500/80 text-white' : 'bg-primary/80 text-black'}`}>
...
export default function AdminPage() {
  ...
  const [selectedCustomerForArchive, setSelectedCustomerForArchive] = useState<string | null>(null);
  const [isZipping, setIsZipping] = useState(false);
  const [previewImage, setPreviewImage] = useState<string | null>(null);
  
  // Health Check State
  const [healthStatus, setHealthStatus] = useState<{db: boolean, storage: boolean, message: string, bucketName?: string} | null>(null);
  ...
  const customers = useMemo(() => {
    ...
    // 1. Initialize with registered users so even those with 0 orders appear
    ...
    // 2. Process orders to calculate LTV and capture guests
    if (Array.isArray(orders)) {
        orders.forEach(order => {
            const customerId = order.customerId;
            
            if (!customerMap.has(customerId)) {
                // This captures Guests OR users who might not be in the passed users list for some reason
            ...
            if (customer.lastOrderDate === 'N/A' || new Date(order.orderDate) > new Date(customer.lastOrderDate)) {
                 customer.lastOrderDate = order.orderDate;
            }
        });
    }

    // Sort orders within each customer for the detail view
    customerMap.forEach(customer => {
        customer.orders.sort((a: Order, b: Order) => new Date(b.orderDate).getTime() - new Date(a.orderDate).getTime());
    });
  ...
  const handleDownloadAllZip = async (targetOrder: Order) => {
    ...
    try {
        ...
        // Loop through items and add to zip
        for (let i = 0; i < targetOrder.items.length; i++) {
            ...
            if (urlToUse) {
                if (urlToUse.startsWith('data:')) {
                    ...
                    const fileName = `Sheet-${i + 1}-${targetOrder.orderId || 'NoID'}.png`;
    ...
    const htmlContent = `
        ...
        <body>
            <div class="header">
                ...
                <div class="order-details"><h2>#${targetOrder.orderId}</h2><p>Date: ${new Date(targetOrder.orderDate).toLocaleDateString()}</p></div>
            </div>
            <div class="section">
            ...
            <div class="section">
                ...
                <table>
                    ...
                    <tbody>${targetOrder.items.map((item, idx) => `<tr><td>${idx+1}</td><td>Gang Sheet ${item.sheetSize.name}</td><td>${item.quantity}</td></tr>`).join('')}</tbody>
                </table>
            </div>
  ...
  const renderContent = () => {
    // 1. CUSTOMER DETAIL ARCHIVE VIEW
    if (viewMode === 'customers' && selectedCustomerForArchive) {
        const customer = customers.find(c => c.id === selectedCustomerForArchive);
                ...
                <div className="flex items-center mb-6">
                    ...
                    <div>
                        ...
                        <div className="flex items-center text-sm text-zinc-500 space-x-4 mt-1">
                            ...
                            <span>•</span>
                            <span>LTV: ${customer.totalSpend.toFixed(2)}</span>
                            <span>•</span>
                            <span>Total Orders: {customer.orderCount}</span>
                        </div>
                    </div>
                </div>

                <div className="flex-grow overflow-y-auto space-y-4 pr-2 builder-scroll">
                    {customer.orders.length > 0 ? customer.orders.map((order: Order) => (
                        <div key={order.id} className="glass-panel rounded-xl border border-white/10 overflow-hidden">
                             <div className="p-4 bg-zinc-800/30 border-b border-white/5 flex justify-between items-center">
                                 <div className="flex items-center space-x-4">
                                     ...
                                     <div>
                                        <h3 className="text-white font-bold text-sm">Order #{order.orderId}</h3>
                                        <p className="text-zinc-500 text-xs">{new Date(order.orderDate).toLocaleString()}</p>
                                     </div>
                                 ...
                                 <div className="flex items-center space-x-2">
                                     <button onClick={() => handlePrintPackingSlip(order)} className="p-2 hover:bg-white/5 rounded-lg text-zinc-400 hover:text-white transition-colors"><Printer className="w-4 h-4" /></button>
                                     <button onClick={() => handleDownloadAllZip(order)} className="px-3 py-1.5 bg-zinc-800 border border-white/10 rounded-lg text-xs font-medium text-white hover:bg-zinc-700 flex items-center"><Download className="w-3 h-3 mr-2" /> Download Assets</button>
                                 </div>
                             </div>
                             <div className="p-4 bg-black/20">
                                 <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                                     {Array.isArray(order.items) && order.items.map((item, idx: number) => (
                                         <div key={idx} className="relative aspect-square bg-checkerboard-dark rounded-lg border border-white/5 overflow-hidden group cursor-zoom-in" onClick={() => setPreviewImage(item.compositeImageUrl)}>
                                             <img src={item.compositeImageUrl} className="w-full h-full object-contain" alt="" />
                                         </div>
                                     ))}
                                 </div>
                             </div>
            ...
            <div className="flex-grow overflow-y-auto builder-scroll">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {customers.map((customer) => (
                        <div key={customer.id} onClick={() => setSelectedCustomerForArchive(customer.id)} className="glass-panel p-6 rounded-2xl border border-white/10 hover:border-primary/30 transition-all cursor-pointer group">
                            <div className="flex items-start justify-between mb-4">
                                ...
                                <div className="text-right">
                                    <p className="text-xs text-zinc-500 uppercase tracking-wider font-bold">LTV</p>
                                    <p className="text-lg font-bold text-white">${customer.totalSpend.toFixed(2)}</p>
                                </div>
                            </div>
                            <h3 className="text-lg font-bold text-white mb-1 truncate">{customer.name}</h3>
                            <p className="text-zinc-500 text-sm mb-4 truncate">{customer.email}</p>
                            <div className="flex items-center justify-between pt-4 border-t border-white/5 text-sm">
                                <span className="text-zinc-400 flex items-center"><History className="w-4 h-4 mr-2" /> {customer.orderCount} Orders</span>
                                <span className="text-primary text-xs font-bold flex items-center group-hover:translate-x-1 transition-transform">View Archive <ChevronRight className="w-4 h-4 ml-1" /></span>
                            </div>
                        </div>
                    ))}
                    {customers.length === 0 && (
    ...
    // 3. DEFAULT: LIVE ORDERS VIEW
    return (
      <div className="glass-panel rounded-2xl overflow-hidden flex-grow flex flex-col border border-white/10 shadow-2xl">
        <ImagePreviewModal isOpen={!!previewImage} onClose={() => setPreviewImage(null)} imageUrl={previewImage} title="Production File Preview" />
        <div className="overflow-auto flex-grow">
            <table className="min-w-full divide-y divide-white/5">
            <thead className="bg-black/20 sticky top-0 backdrop-blur-md z-10">
                <tr>
                    <th className="px-6 py-4 text-left text-xs font-bold text-zinc-400 uppercase tracking-wider cursor-pointer" onClick={() => handleSort('date')}>Order Date</th>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                            ...
                            <div className="ml-4">
                                ...
                                <div className="text-xs text-zinc-500">{new Date(order.orderDate).toLocaleDateString()}</div>
                            </div>
                        </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-zinc-200">{order.customerName}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-xs font-medium text-white bg-zinc-800 px-2 py-1 rounded border border-white/5">{order.items[0].sheetSize.name}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm font-bold text-white">${(order.total || 0).toFixed(2)}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-3 py-1 inline-flex text-xs leading-5 font-bold rounded-full border ${getStatusColor(order.status)}`}>{order.status}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <button className="text-zinc-400 hover:text-white p-2" onClick={(e) => { e.stopPropagation(); setSelectedOrderId(order.orderId); }}><ChevronRight className="w-5 h-5" /></button>
                    </td>
                </tr>
                )) : (
                    <tr><td colSpan={6} className="px-6 py-12 text-center text-zinc-500"><Search className="w-12 h-12 mx-auto mb-3 opacity-20" /><p>No orders found.</p></td></tr>
                )}
            </tbody>
            </table>
        </div>
    ...
    <div className="flex flex-col h-full max-w-7xl mx-auto w-full px-4 py-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
                ...
                <p className="text-zinc-400 text-sm mt-1">Manage orders and customer data</p>
            </div>
            ...
            <div className="flex items-center space-x-4">
                ...
                >
                    {isCheckingHealth ? (
                        ...
                        <span className="flex items-center"><Cloud className={`w-4 h-4 mr-2 ${healthStatus?.storage ? 'text-accent' : 'text-zinc-500'}`} /> System Health</span>
                    )}
                </button>
                        ...
                        {healthStatus.storage && healthStatus.db ? (
                            <>
                                ...
                                {healthStatus.bucketName && (
                                    <span className="text-zinc-500 text-[10px] mt-0.5 max-w-[150px] truncate" title={healthStatus.bucketName}>
                                        Bucket: {healthStatus.bucketName}
                                    </span>
                                )}
                            </>
                        ) : (
                            <span className="text-red-500 font-bold">● {healthStatus.message}</span>
                        )}
                    </div>
                )}
                ...
                <div className="bg-zinc-900 p-1 rounded-lg border border-white/10 flex">
                    <button 
                        onClick={() => { setViewMode('orders'); setSelectedCustomerForArchive(null); }}
                        className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${viewMode === 'orders' ? 'bg-zinc-800 text-white shadow-lg' : 'text-zinc-400 hover:text-white'}`}
                    >
                        Live Orders
                    </button>
                    <button 
                        onClick={() => { setViewMode('customers'); setSelectedCustomerForArchive(null); }}
                        className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${viewMode === 'customers' ? 'bg-zinc-800 text-white shadow-lg' : 'text-zinc-400 hover:text-white'}`}
                    >
                        Customers
                    </button>
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="md:col-span-2 relative">
                ...
                <input 
                    ...
                    placeholder={viewMode === 'orders' ? "Search by Order ID, Customer Name, or Tracking ID..." : "Search by Name or Email..."}
            ...
            {viewMode === 'orders' && (
                <div className="flex space-x-2">
                    ...
                    >
                        ...
                        {Object.values(statusWorkflow).map(s => <option key={s} value={s}>{s}</option>)}
                    </select>
                </div>
            )}
        </div>
